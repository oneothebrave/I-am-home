...
`signed/到家了么通知.shortcut` 是从 macOS “快捷指令”编辑器选择“导出给任何人”
生成的 Apple 签名产物。重新共享会生成新的 iCloud 标识；更新模板后，必须同时更新
`src/native/shortcutNotification.ts` 和对应测试，且应先在真机完成安装与发送测试。
